...
import UserListAddRow from './UserListAddRow.vue'
  ...
  <div class="datatable">
    ...
    <v-data-table :items="tableItems" :headers="headers" item-key="id" :search="search">
      <template v-if="newUser" #[`body.prepend`]>
        <UserListAddRow :item="newUser" @save="addUser" @cancel="newUser = null" />
      </template>
      <template #item="{ item }">
        <UserListRow :item="item" @save="saveUser" @delete="onDeleteItem" />
      </template>
    </v-data-table>
  </div>
</template>
